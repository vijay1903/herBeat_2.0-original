  ...
  .config(['ChartJsProvider', '$routeProvider', '$httpProvider', function (ChartJsProvider, $routeProvider, $httpProvider) {
    ...
    $scope.$watch('username', function(newVal, oldVal){
        if(newVal!=oldVal){
            ...
            $scope.startDate = new Date("04/02/2018");
  ...
  function getByPatientId(val){
      ...
      $http({
        ...
        .success(function(data) {
            ...
            //         $scope.emaResp.push({serial_no: i+1, user_selected_activity: final_data[i].user_selected_activity,
            //             user_company: final_data[i].user_company, user_curr_location: final_data[i].user_curr_location,
            //             user_food_habit: final_data[i].user_food_habit, user_feelings: final_data[i].user_feelings,
            //             motivation_screen: final_data[i].motivation_screen,
            //             activity_date: final_data[i].date,
            //             activity_time: final_data[i].time
            //             });
            //     }
              
            //     console.log("response array : ", final_data)
            //     $scope.searchCount = final_data.length;
            //     $scope.search();
            // }
  ...
  function searchEmaResp(val){
            ...
            $scope.searchBy = function(){
                ...
                if($scope.all) {
                    ...
                    for(var a in $scope.act_filter_options){
                        $scope.searchText['user_selected_activity'] = $scope.act_filter_options[a].label;
                        if($scope.act_filter_options[a].flag){
                            temp = temp.concat($filter('filter')(out,$scope.searchText,true)).unique();
                        }
                    }
                    out = temp;
                    temp = [];
                    // console.log('out after step 1: ',out);
                    for(var a in $scope.comp_filter_options){
                        $scope.searchText['user_company'] = $scope.comp_filter_options[a].label;
                        if($scope.comp_filter_options[a].flag){
                            temp = temp.concat($filter('filter')(out,$scope.searchText,true)).unique();
                        }
                    }
                    out = temp;
                    temp = [];
                    // console.log('out after step 2: ',out);
                    for(var a in $scope.loc_filter_options){
                        $scope.searchText['user_curr_location'] = $scope.loc_filter_options[a].label;
                        if($scope.loc_filter_options[a].flag){
                            temp = temp.concat($filter('filter')(out,$scope.searchText,true)).unique();
                        }
                    }
                    out = temp;
                    temp = [];
                    // console.log('out after step 3: ',out);
                    for(var a in $scope.food_filter_options){
                        $scope.searchText['user_food_habit'] = $scope.food_filter_options[a].label;
                        if($scope.food_filter_options[a].flag){
                            temp = temp.concat($filter('filter')(out,$scope.searchText,true)).unique();
                        }
                    }  
                    out = temp;
                    temp = [];
                    // console.log('out after step 4: ',out);
                    for(var a in $scope.feel_filter_options){
                        $scope.searchText['user_feelings'] = $scope.feel_filter_options[a].label;
                        if($scope.feel_filter_options[a].flag){
                            temp = temp.concat($filter('filter')(out,$scope.searchText,true)).unique();
                        }
                    }
                    out = temp;
                    // console.log('out after all steps: ',out);
                    final_data = out; 
                } else { 
                    final_data = []; 
                }
